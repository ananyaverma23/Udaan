  ...
  return (
    <div className="fixed inset-0 z-[200] bg-white flex flex-col items-center justify-center p-4 overflow-hidden">
      ...
      <div className="text-center w-full max-w-xs sm:max-w-md">
        <h1 className="text-4xl sm:text-5xl font-bold text-teal-600 tracking-tight mb-2">
          UDAAN
        </h1>
          ...
          >
            {messages.map((msg, idx) => (
              <div key={idx} className="h-10 flex items-center justify-center">
                <span className="bg-[#F0F7F4] text-teal-600 px-5 py-1.5 rounded-full text-xs sm:text-sm font-bold border border-[#F0F7F4] shadow-sm">
                  {msg}
                </span>
              </div>
            ))}
